class RayData {
    constructor(map, origin, direction, target, maxDistance, materialRef, sx, sy, sz, tx, ty, tz, dx, dy, dz, vx, vy, vz) {
        ...
        this.getRayIntersectionPoint = () => {
            if(vx) {
                const t = (target.x - origin.x) / direction.x;
                //const t = ((tx / dx) * sx) / direction.x;
            ...
            if(vy) {
                const t = (target.y - origin.y) / direction.y;
                //const t = ((ty / dy) * sy) / direction.y;
                const intersection = new Point(
            ...
            const t = (target.z - origin.z) / direction.z;
            //const t = ((tz / dz) * sz) / direction.z;
            const intersection = new Point(
                direction.x * t + origin.x,
                direction.y * t + origin.y,
                t,
