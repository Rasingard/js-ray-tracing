class Map {
    constructor(sX, sY, sZ) {
        const buffer = new SharedArrayBuffer(Uint8Array.BYTES_PER_ELEMENT * sX * sY * sZ);
        const _root = new Uint8Array(buffer);
        ...
        const bufMaterials = new MaterialList();

        this.addMaterial = (material) => {
            material.index = materials.length;
            materials.push(material);

            bufMaterials.add(material.color, 253, 254, 255);
            return material.index;
        }
        this.getMaterial = (ref) => materials[ref];

        // Lights
        const globalLights = new Array();
        this.addGlobalLight = (light) => {
            const index = globalLights.length;
            globalLights.push(light);
            return index;
        }
        this.addGlobalLight(
            new GlobalLight(new Color(255, 250, 175), 0.75, new Vector(0, -1, 0))
        );
        this.getGlobalLights = () => globalLights;

        const ambientLight = new AmbientLight(new Color(132, 235, 255), 0.25);
        this.getAmbientLight = () => ambientLight;

        const materialsBuffer = new MaterialList();
        const texturesBuffers = new TextureList();
        this.addTextureBuffer = (buffer) => texturesBuffers.add(buffer);
        this.addMaterialBuffer = (color, textureTRef, textureSRef, textureDRef) => materialsBuffer.add(color, textureTRef, textureSRef, textureDRef);
    }
