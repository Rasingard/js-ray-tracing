class CameraViewer {
    ...
    render3(map, height, width, fov) {
        const image = new ImageData(width, height);
        const transformM4 = Matrix4.tranformation(this.location, this.axis.x, this.axis.y, this.axis.z);
        ...
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                targetShader = map.rayTrace(this.location, vectorTo(x, y));

                if (targetShader) this.changePixelColor(image, x, y, targetShader.render(map, SUN));
                else this.changePixelColor(image, x, y, this.defColor);
            }
        }
