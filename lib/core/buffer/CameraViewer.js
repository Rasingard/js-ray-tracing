class CameraViewer {
    ...
    renderRay(i, image, v, rayData) {
        let sh, sw, skySI = 0, skyColor = new Color(0,0,0);

        if (rayData) {
            this.changeBufferPixelColor(image, i, Shader.renderRayData(rayData, this.skyData, this.skyHeight, this.skyWidth));
        } else {
    ...
    renderToBuffer(buffer, map, height, width, fov, renderDistance, sx, sy) {
        ...
        for(let f = 0; f < flowsCount; f++) {
            ...
            for(let i = (skip * 4) + (16 * f), len = image.length; i < len; i += step) {
                ...
                this.renderRay(i, image, v, rayData);
            }
        }
