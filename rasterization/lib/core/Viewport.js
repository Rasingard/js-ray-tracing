class Viewport {
    ...
    drawLine(image, x0, y0, x1, y1) {
        ...
        while(true) {
            if(x0 >= 0 && x0 < image.width && y0 >= 0 && y0 < image.height) this.changePixelColor(image, x0, y0, new Color(0, 255, 0));
    ...
    polyFill(image, p1, p2, p3, color) {
        ...
        const m = (p3.x - p1.x) / (p3.y - p1.y);
        const p4 = { x: ((p2.y - p1.y) * m) + p1.x, y: p2.y };

        let ml, mr, px, py;

        if(p2.x > p4.x) {
            ml = (p4.x - p1.x) / (p2.y - p1.y);
            mr = (p2.x - p1.x) / (p2.y - p1.y);

            for(let y = 0; y < (p2.y - p1.y); y++){
                for(let x = ((y * ml) + p1.x), len = ((y * mr) + p1.x); x < len; x++) {
                    px = Math.floor(x);
                    py = Math.floor(y + p1.y);

                    if(px < 0 || px >= image.width || py < 0 || py >= image.height) continue;
                    this.changePixelColor(image, px, py, color);
                }
            }

            ml = (p3.x - p4.x) / (p3.y - p2.y);
            mr = (p3.x - p2.x) / (p3.y - p2.y);

            for(let y = 0; y < (p3.y - p2.y); y++){
                for(let x = ((y * ml) + p4.x), len = ((y * mr) + p2.x); x < len; x++) {
                    px = Math.floor(x);
                    py = Math.floor(y + p2.y);
    ...
    renderPoly(image, p1, p2, p3, matrix) {
        ...
        if(p1.x < 0 || p1.x > image.width) return;
        p1.y = h2 + (p1.y * image.width) / p1.z;
        if(p1.y < 0 || p1.y > image.height) return;

        p2.x = w2 + (p2.x * image.width) / p2.z;
        if(p2.x < 0 || p2.x > image.width) return;
        p2.y = h2 + (p2.y * image.width) / p2.z;
        if(p2.y < 0 || p2.y > image.height) return;

        p3.x = w2 + (p3.x * image.width) / p3.z;
        if(p3.x < 0 || p3.x > image.width) return;
        p3.y = h2 + (p3.y * image.width) / p3.z;
        if(p3.y < 0 || p3.y > image.height) return;
    ...
    raster(map, camera, points, polys, width, height) {
        ...
        for(let i = 0, len = polys.length; i < len; i += 3) {
            ...
            this.renderPoly(image, p1, p2, p3, matrix);
        }
